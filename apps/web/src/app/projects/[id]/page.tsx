...
export default async function ProjectPage({
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link href="/backlog" className="link" style={{ fontSize: 13 }}>
          ← Backlog
        </Link>
        <div className="mt-2 flex flex-wrap items-center gap-3">
          <h1 className="v3-headline">{project.title}</h1>
        ...
        <div className="mt-4 v3-card" style={{ padding: 12 }}>
          <GatesProgress
      ...
      <div className="v3-subnav" style={{ position: "static", margin: "0 0 4px" }}>
        {TABS.map((t) => (
          <Link
            key={t.id}
...
function Overview({
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
      <div className="v3-card v3-card-pad lg:col-span-2">
        <h2 style={{ fontSize: 15, fontWeight: 600, marginBottom: 8 }}>
          Problem statement
        </h2>
        <p className="whitespace-pre-wrap" style={{ fontSize: 13.5, color: "var(--ink-2)" }}>
  ...
  if (requiredByGate.size === 0) {
    return (
      <div className="v3-empty">
        ...
        <div className="msg">No approvals required</div>
        <div className="sub">This tier has no required reviewer gates.</div>
      </div>
    );
  }
    ...
    <section className="v3-card" style={{ overflow: "hidden" }}>
      ...
      >
        <div className="flex items-center gap-3">
          <h3 style={{ fontSize: 14, fontWeight: 600 }}>{gate}</h3>
      ...
      <ul style={{ borderTop: "none" }} className="divide-y divide-surface-divider">
        {subs.map((a) => {
          const overdue =
            a.status === "Pending" &&
            a.slaDueAt &&
        ...
        <table className="v3-data-table">
          ...
          <tbody>
            {entries.length === 0 ? (
              <tr>
                <td colSpan={4}>
                  <div className="v3-muted" style={{ padding: "32px 0", textAlign: "center", fontSize: 13 }}>
                    No entries yet.
                  </div>
                </td>
              </tr>
              ...
              entries.map((e) => (
                <tr key={e.id}>
                  <td data-label="ID" className="mono">{e.testCaseId}</td>
                  <td data-label="Phase">{e.phase}</td>
                  <td data-label="Scenario">{e.scenario}</td>
  ...
  return (
    <div className="space-y-4">
      ...
      {list.length === 0 ? (
        <div className="v3-empty">
          <div className="icon">⎘</div>
          <div className="msg">No artifacts uploaded yet</div>
        </div>
      ) : (
        <ul className="v3-card divide-y divide-surface-border">
          {list.map((a) => (
            <li key={a.id} className="flex items-center justify-between p-4 text-sm">
              <span style={{ fontWeight: 500 }}>{artifactTypeLabel(a.type)}</span>
              <a href={a.blobUrl} target="_blank" rel="noreferrer" className="link">
                v{a.version} ↗
              </a>
            </li>
          ))}
  ...
  return (
    <div className="space-y-4">
      ...
      {links.length === 0 ? (
        <div className="v3-empty">
          <div className="icon">⛓</div>
          <div className="msg">No solution links yet</div>
        </div>
      ) : (
        <ul className="v3-card divide-y divide-surface-border">
          {links.map((l) => (
            ...
            >
              <div>
                <div className="v3-label-uc">{linkTypeLabel(l.linkType)}</div>
                <div style={{ fontWeight: 500 }}>{l.label ?? l.url}</div>
              </div>
              <a href={l.url} target="_blank" rel="noreferrer" className="v3-btn-outline v3-btn-sm">
                Open ↗
              </a>
            </li>
          ))}
...
function ShowcaseTab({
  ...
  return (
    <div className="v3-card v3-card-pad space-y-4">
      <div className="flex items-baseline justify-between">
        <h2 style={{ fontSize: 15, fontWeight: 600 }}>Showcase preview</h2>
        <a href={`/api/showcase/${projectId}.pdf`} className="v3-btn-primary">
          Download PDF
        </a>
      </div>
      <div
      ...
      >
        ...
        <p style={{ fontSize: 13, color: "var(--ink-2)" }}>{project.problemStatement}</p>
        {activeVersion && (
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div>
              <div className="v3-label-uc">Annual $</div>
              <div className="mono" style={{ fontWeight: 600 }}>
                {formatUsd(activeVersion.annualSavedUsd)}
              </div>
            </div>
            <div>
              <div className="v3-label-uc">Hours saved</div>
              <div className="mono" style={{ fontWeight: 600 }}>
                {formatNumber(activeVersion.annualSavedHours)}
              </div>
            </div>
            <div>
              <div className="v3-label-uc">Quality $</div>
              <div className="mono" style={{ fontWeight: 600 }}>
                {formatUsd(activeVersion.qualityValueUsd)}
              </div>
            </div>
            <div>
              <div className="v3-label-uc">Implemented</div>
              <div className="mono" style={{ fontWeight: 600 }}>
...
function Field({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div>
      <div className="v3-label-uc">{label}</div>
      <div className="mt-0.5" style={{ color: "var(--ink-2)", fontSize: 13.5 }}>
        {value}
      </div>
    </div>
  );
...
function Row({ k, v }: { k: string; v: React.ReactNode }) {
  return (
    <div
      className="flex justify-between py-1.5 last:border-b-0"
      style={{ borderBottom: "1px solid var(--hairline)" }}
    >
      <dt className="v3-muted">{k}</dt>
